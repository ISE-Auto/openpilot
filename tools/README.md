...
### REQUIREMENTS ON TOP OF STATED REQUIREMENTS IN README ###

Run these after installing the other requirements.

`sudo apt-get install opencl-headers scons libgles2-mesa-dev=18.0.0~rc5-1ubuntu1 ocl-icd-opencl-dev`

`sudo mkdir /usr/include/KHR`
...
`mv khrplatfotm.h /usr/include/KHR/`

symlink the libGL so openpilot will compile

`sudo ln -s /usr/lib/x86_64-linux-gnu/libGL.so /usr/lib/x86_64-linux-gnu/libGL.so`
...
-------------
___
Before running this, compile the requirements.
...
scons
```
Hopefully it doesn't fail. Now, back to the guide.
